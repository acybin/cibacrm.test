...
class datatable extends node
{
    ...
    protected $_tfoot = '';
    protected $_striped = true;
    protected $_responsive = true;
    
    public function __construct()
    {
        parent::__construct('table');
        $this->getAttributes()->getClass()->setItems(array('table', 'table-bordered', 'nowrap', 'datatable'));
        $this->getAttributes()->addAttr('width', '100%');
    }
    ...
    protected function refresh($property)
    {
        if ($property == '_thead')
        {
            ...
            {
                $th_node = new node('th');
                if (!is_numeric($key)) $th_node->getAttributes()->getClass()->addItems($key);
    ...
    public function __toString()
    {
        if ($this->_striped)
        {
            $this->getAttributes()->getClass()->addItems('table-striped');    
        }
        
        if ($this->_responsive)
        {
            $this->getAttributes()->getClass()->addItems('dt-responsive');    
        }
